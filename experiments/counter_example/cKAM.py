...
idx_schedule = idx_scheduler(
    iterations_cycle, subsample_size=50, decay_rate=0.4)
...
samples_cKAM, avg_acc, samples_index_array_cKAM, stepsize_array_cKAM = \
    cKAM(counter_posterior, prior, X_INIT, iterations_cycle, n_cycles, beta,
         [], acceptance, initial_stepsize, gradk, idx_schedule, noise_schedule,
         eps=2, optim_acc=0.234)

np.save(f'{RESULTS_DIR}{os.sep}samples_cKAM_counter_example.npy', samples_cKAM)
np.save(f'{RESULTS_DIR}{os.sep}samples_index_array_cKAM.npy', samples_index_array_cKAM)
np.save(f'{RESULTS_DIR}{os.sep}stepsize_array_cKAM.npy', stepsize_array_cKAM)
