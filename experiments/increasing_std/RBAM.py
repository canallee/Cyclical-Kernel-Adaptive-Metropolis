...
samples_RBAM, avg_acc, accepted_RBAM, samples_index_array_RBAM = Rao_Blackwellised_AM(
    log_posterior, prior, x_init, 2000000, 800000, [], acceptance)


np.save(f'{RESULTS_DIR}{os.sep}samples_RBAM_{exp_name}.npy', samples_RBAM)
np.save(f'{RESULTS_DIR}{os.sep}accepted_RBAM_{exp_name}.npy', accepted_RBAM)
np.save(f'{RESULTS_DIR}{os.sep}samples_index_array_RBAM_{exp_name}.npy', samples_index_array_RBAM)
